...
if [ "$ELASTICSEARCH_SERVICE_HOST" != "<will be defined>" ]; then
	RET=1
	while [ $RET -ne 0 ]; do
		echo "Checking if $ELASTICSEARCH_SERVICE_HOST is available."
		curl -XGET "$ELASTICSEARCH_SERVICE_HOST:$ELASTICSEARCH_SERVICE_PORT/_cat/health?v&pretty" > /dev/null 2>&1
		RET=$?
...
if [[ -d /var/www/html/vendor/magento ]]; then
	...
	bin/magento setup:install \
		...
		--elasticsearch-host=$ELASTICSEARCH_SERVICE_HOST \
		--elasticsearch-port=$ELASTICSEARCH_SERVICE_PORT \
